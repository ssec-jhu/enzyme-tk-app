...
Six rules the skeleton encodes:
...
- **Success is a component; every error is a string.** They share one `Output`, and [`07-modals.css`](../enzyme_tk_app/app/assets/07-modals.css) tells them apart by *type* — a bare string in the submission-results div is painted red, while `build_submission_success(job_id, detail)` brings its own green box and the `/my-tasks` link (the modal stays open after Run, so that link is the only signpost to the page tracking the job). Return a success as an f-string and it renders as an error; wrap an error in a component and it renders as a success. `test_every_tool_links_to_my_tasks_on_success` in `test_tools.py` walks the live registry for the import, and the two behavioural tests in `test_tools_timer.py` pin both return types. Full rules in [`create-modal`](../.claude/agents/create-modal.md) §9.
