...
| **Input parameters table** | `build_result_input_params()` | Auto-rendered from `job.params`. SMILES values get inline structure previews; a `databases` list is rendered as comma-separated filenames, extensions included (`_DATABASE_PARAM_KEYS`), not a list repr. Use `_params_exclude` to hide internal keys. |
