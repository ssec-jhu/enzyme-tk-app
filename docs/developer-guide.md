...
**Never read `data/` from a test — with one exception.** The real databases and checkpoints are gitignored, so a test that reads them passes only on a machine that happens to have them and fails in CI. Write the fixture files your test needs into `tmp_path` and monkeypatch the path constant at it. The exception is the **demo sets**, which are tracked and therefore present in CI: `test_demo_data.py` reads them deliberately, because its whole job is to assert on what actually ships — that every tool example still finds its row. Such a test must assert **containment** (`"enzymes_demo_set.tsv" in names`), never equality: a developer machine also has the full sets in those directories, so an equality assertion would pass only on a fresh clone. Patch it **where it was imported**, not in `paths.py` — the importing module bound its own name at import time:
