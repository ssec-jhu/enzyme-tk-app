...
- **Every tool's `submit_*` callback returns one of two shared blocks — never a bare string.** On success, `build_submission_success(job_id, detail)` from `components/modal_helpers.py`; on every validation failure, `build_submission_error(message)`. Both are **one line** and share the `modal-submission-row` base class in `07-modals.css`, so the two states are the same height and shape by construction and differ only in colour and icon — green with the `/my-tasks` link, red with a cross. The success row shows the job id through the shared `truncate_id()` with the full value in its tooltip (the same 6-character form the My Tasks table and `/admin` show) and a terse detail fragment (`"2 database(s) · top 10"`, not a sentence), because the link is the only thing telling a user their job is now tracked somewhere and the modal deliberately stays open after Run. They share **one** `Output` on `f"id-div-{TOOL_DEF['slug']}-results"`, which is a **plain slot with no styling of its own** — so a message returned bare renders with no box, no colour and no icon, reading as ordinary body text. Only the clear-on-reopen branch returns `""`, which leaves the slot empty and invisible. `test_every_tool_links_to_my_tasks_on_success` and `test_every_tool_wraps_its_submit_errors` in `tests/test_tools.py` walk the live registry for both imports; `test_submit_success_links_to_my_tasks` and `test_submit_error_returns_the_shared_error_row` in `test_tools_timer.py` pin both return shapes, and `conftest.py`'s `submitted_job_id()` / `submission_error_text()` are how a test reads each. Full rationale in `create-modal` §9.
