...
- **Every tool's `submit_*` callback returns the shared success block; every error return stays a bare string.** On success it returns `build_submission_success(job_id, detail)` from `components/modal_helpers.py` — a **single line** — the job id through the shared `truncate_id()` with the full value in its tooltip, a terse detail fragment (`"2 database(s) · top 10"`, not a sentence), and the `/my-tasks` link that is the only thing telling a user their job is now tracked somewhere, since the modal deliberately stays open after Run. Success and error share **one** `Output` on `f"id-div-{TOOL_DEF['slug']}-results"` and `07-modals.css` tells them apart **by type**: a bare string is painted red as a validation error, the block brings its own green box and a `:has()` rule cancels the red one around it. The distinction is load-bearing in both directions — a success returned as an f-string renders as an error, an error wrapped in a component renders as a success, link and all — and the clear-on-reopen branch stays `return ""`. `test_every_tool_links_to_my_tasks_on_success` in `tests/test_tools.py` walks the live registry for the import; `test_submit_success_links_to_my_tasks` and `test_submit_error_stays_a_plain_string` in `test_tools_timer.py` pin the two return types the import walk cannot see. Full rationale in `create-modal` §9.
