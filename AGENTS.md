...
- **A Func-E reaction takes exactly one molecule per side of `>>`.** `funce/compute._encode_reaction` splits on `>>` and hands each *whole* side to UniMol as one structure to build a 3D conformer from, so a dot-joined side (`A.B`) is embedded as a single nonsense molecule. It does not fail loudly: unimol_tools runs in `mode="fast"`, where a failed `AllChem.EmbedMolecule` falls back to `Compute2DCoords`, so the job finishes as a complete, successful-looking ranking built on a meaningless vector. Omit leaving groups (DEHP → MEHP omits the released 2-ethylhexanol). No validator enforces this — `test_every_example_reaction_has_one_molecule_per_side` in `tests/test_tools_funce.py` pins it for the *shipped examples* only. The constraint is **Func-E's alone**: `reaction_similarity` and `substrate_product_similarity` use dot-joined sides freely because RDKit fingerprints do not care.
