...
- [ ] **Exception — do not merge or reorder the three Python install steps.** They run torch (CPU wheel, `ARG TORCH_INDEX_URL`) → `requirements.txt` → `rxnfp --no-deps`, and the order is load-bearing: torch must come first so that `enzymetk`'s unpinned `torch` dependency is already satisfied (otherwise pip resolves the CUDA build and adds ~2.9 GB of `nvidia-cu*` wheels on amd64), and `rxnfp` must stay separate because `--no-deps` cannot be expressed per-package inside a requirements file. Each has a comment block stating why; read it before editing. **The same trap is enforced a second time outside this file:** `tox.ini`'s `[testenv:test]` sets `PIP_EXTRA_INDEX_URL` to the CPU channel on Linux, because CI installs `requirements/prd.txt` with no Dockerfile ordering to protect it and the CUDA stack overruns a GitHub runner's ~14 GB disk. Same invariant, different mechanism (an extra index, not install order) — changing how torch is installed here does not fix CI, and vice versa, so check both.
- [ ] Pin all versions — base image tags, binary releases, `COPY --from` image tags. No `latest` or floating tags. **One deliberate exception:** `requirements/prd.txt` tracks `enzymetk` by branch (`@funce-updates`) until it has a release. Both pip and the layer cache see only the branch name, so `docker compose build --no-cache` is how a build picks up new commits (`docs/deployment-guide.md` → *Refetching `enzymetk`*), matching what `scripts/db_build/` already documents for the same branch. Do not add a cache-buster `ARG` for this — a build arg that exists only to be given a throwaway value is a new mechanism where `--no-cache` already works, and it would be dead config the day `enzymetk` gets a version.
- [ ] `COPY --from` stage aliases go at the **top** of the Dockerfile, before the main `FROM` line.
- [ ] **The build context is filtered by `.dockerignore` alone — read it before adding or widening a `COPY`.** The closing `COPY . .` under `WORKDIR /app` takes everything the context still holds, and *gitignored is not excluded*: until `.env` and `.env.*` were listed there, a local `docker compose build` on a machine with a real `.env` baked `ETK_ADMIN_TOKEN` and `ETK_SECRET_KEY` into the image at `/app/.env` as a published layer. CI never saw it — `docker/build-push-action` builds from a clean checkout — so this is a class of bug a green pipeline cannot catch. Nothing in the image needs those values: there is no `python-dotenv` anywhere in the repo, every setting is an `os.environ.get` (`backend/config.py`, `backend/celery_app.py`, `paths.py`, `utils/submission_limits.py`, `utils/captcha.py`), and `docker-compose.yml` does its `${VAR}` substitution on the **host**. So never `COPY` a secret in, and never pass one as an `ARG` or `ENV` — both are recorded in the image history for anyone who pulls the tag. `Makefile` (which sources `.env` on the host) and `scripts/*` (which holds the committed `scripts/template.env` placeholder) are already excluded. Adding a `COPY` of a whole directory is the risky case; confirm with `docker run --rm <image> ls -a /app` after the build. The operator-facing statement of this lives in `docs/deployment-guide.md` → *Admin Secrets*.
