...
| `enzyme_tk_app/app/utils/captcha.py` (policy constant, challenge path, or what `APP_IN_PRODUCTION_MODE` gates) | `docs/deployment-guide.md` → Production Mode → *Submission captcha* (and the `ETK_SECRET_KEY` / `APP_IN_PRODUCTION_MODE` rows above it), `docs/admin-login.md` (the key is derived from `ETK_SECRET_KEY`), `AGENTS.md`'s submit-callback contract, and `create-tool` / `create-modal` / `write-callback` / `write-tests`. It reads **no** variable of its own, so `template.env`/compose/bicep stay untouched — say so rather than adding a row nobody sets. Key-derivation and secret wording is `admin-page`'s to approve |
| a **vendored** third-party asset under `enzyme_tk_app/app/assets/` (e.g. `11-altcha.js`) added, upgraded, or removed | `docs/developer-guide.md` → the dependency paragraph in *Run and Verify*; check the file's own provenance banner (source URL, version, sha256, licence) still matches, and that any PyPI counterpart in `requirements/prd.txt` is still on a compatible major |
...
| `.dockerignore`, or a `Dockerfile` `COPY` that widens the build context | `docs/deployment-guide.md` → Admin Secrets + Building the Image, and `.claude/agents/edit-dockerfile.md` — what may enter a published image is a security boundary, not a build detail. Secret-related wording is `admin-page`'s to approve |
