...
5.  **Callback Testing**
    ...
    - **A submit callback's success return is a component, its errors are plain strings** — so assert on the success through `get_text(result)`, never `in result` (`conftest.py`'s helper flattens a Dash tree to its text; `assert "job-123" in result` on an `html.Div` proves nothing). The type is the contract `07-modals.css` reads, so pin it where a new tool would get it wrong: `assert not isinstance(result, str)` on the success path and `assert isinstance(result, str)` on an error, plus the `/my-tasks` anchor via `find_components(result, html.A)`. **Never assert a job id against the rendered text** — the block puts it through `truncate_id()`, so `"job-rxn-123" in get_text(result)` fails while a prefix passes for the wrong reason; use `conftest.py`'s `submitted_job_id(result)`, which reads the full id out of the tooltip where it actually lives, and assert equality. See `test_submit_success_links_to_my_tasks` and `test_submit_error_stays_a_plain_string` in `test_tools_timer.py`; `test_tools.py`'s `test_every_tool_links_to_my_tasks_on_success` already covers the import for every tool, so do not re-assert it per tool.
