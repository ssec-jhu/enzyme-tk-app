...
- **When writing or editing any CSS in `assets/`, follow the [Write-CSS Agent](agents/write-css.md)** for banner/section-comment style, 4-space indentation, and ~79-column separator padding. In short: top-of-file banners use the `/* === */` three-line form; section separators use a single `/* --- Title ---…--- */` line (never `/*!* … *!*/` or other nonstandard markers); selectors start at column 0 with 4-space (never tab) indentation.
