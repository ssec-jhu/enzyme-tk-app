...
Because the mount is read-only, nothing here can be regenerated by the running app. Three of the
directories below are built from your own sequence file by
[`scripts/db_build/`](scripts/db_build/README.md) — `foldseek_db/`, `sequence_embeddings/`, and the
`foldseek_models/weights/` it downloads on the way.
...
| `unimol_weights/` | Func-E Activity Prediction | The UniMol v2 164M checkpoint at `modelzoo/164M/checkpoint.pt` (~660 MB), used to embed the query reaction's substrate and product. Named for the model, not for its reader. Func-E passes this directory to the UniMol step as `weights_dir`. The exact checkpoint path is checked, not just the directory: the mount is read-only, so a wrong layout cannot heal itself with a download |
...
Use the standalone builder in [`scripts/db_build/`](scripts/db_build/README.md). It takes one CSV or TSV
(plain or gzipped) with `Entry` and `Sequence` columns and produces either or both of the artifacts the
tools read — a FoldSeek database for `data/foldseek_db/` and an ESM3 embeddings pickle for
`data/sequence_embeddings/`. Drop your file in that folder, point `INPUT_FILE` at it, and run; see that
folder's [README](scripts/db_build/README.md) for the full steps.
